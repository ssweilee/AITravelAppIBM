...
exports.toggleSave = async (req, res) => {
  ...
  if (type === 'post') {
    ...
    await user.save();
    return res.json({ saved: idx < 0, count: user.savedPosts.length });
  }
  ...
  if (type === 'trip') {
    ...
    const idx = trip.savedBy.findIndex(u => u.toString() === userId);
    if (idx >= 0) trip.savedBy.splice(idx, 1);
    else          trip.savedBy.push(userId);

    await trip.save();
    return res.json({ saved: idx < 0, count: trip.savedBy.length });
  }
